...
void row_1_other_way() {
  ...
  for (int i = sizeof(segment_4) / 4 - 23; i < sizeof(segment_4) / 4 ; i = i + 3) {
    pixels.setPixelColor(segment_4[(i)], pixels.Color(R_val, G_val, B_val));
  }
...
void row_5_other_way() {
  ...
  for (int i = sizeof(segment_9) / 4 - 2; i > sizeof(segment_9) / 4 - 15; i = i - 3) {
    pixels.setPixelColor(segment_9[(i )], pixels.Color(R_val, G_val, B_val));
  }

  for (int i = sizeof(segment_9) / 4 - 15; i > 0; i = i - 3)  {
    pixels.setPixelColor(segment_9[(i - row_5_count )], pixels.Color(R_val, G_val, B_val));
  }
...
void row_8_other_way() {
  ...
  row_8_count++;
  for (int i = sizeof(segment_6) / 4 ; i >  0; i = i - 3) {
    pixels.setPixelColor(segment_6[(i - row_8_count)], pixels.Color(R_val, G_val, B_val));
  }
...
void row_2_arm_1_other_way() {
  ...
  row_2_arm_1_count++;
  for (int i = 0; i < sizeof(segment_2_arm_1) / 4 - 2; i = i + 3) {
    arm_1.setPixelColor(segment_2_arm_1[(i + row_2_arm_1_count)], arm_1.Color(R_val, G_val, B_val));
  }
